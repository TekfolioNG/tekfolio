<template>
  <footer class="bg-[#012430] text-white pt-16 pb-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <!-- Main Footer Content -->
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-12 text-center md:text-left">
        <!-- Company Info & Logo -->
        <div class="col-span-1 md:col-span-2 lg:col-span-1">
          <div class="mb-6">
            <NuxtLink to="/" class="inline-block">
              <img src="/assets/img/elevation-logo2.png" alt="Syntanium Energy Logo"
                class="h-12 md:h-14 lg:h-16 drop-shadow-lg" />
            </NuxtLink>
          </div>
          <p class="text-gray-300 mb-6 max-w-md mx-auto md:mx-0">
            We are a boutique consulting firm dedicated to strengthening Nigeria's and Africa’s private sector through
            enterprise
            support, capacity development, and strategic advisory.
          </p>
          <div class="flex space-x-4 justify-center md:justify-start">
            <NuxtLink to="https://web.facebook.com/people/Elevation-Consulting"
              class="text-gray-400 hover:text-[#00D4FF] transition-colors duration-200">
              <Icon name="lucide:facebook" class="w-6 h-6" />
            </NuxtLink>
            <NuxtLink to="https://www.linkedin.com/in/elevation-consulting"
              class="text-gray-400 hover:text-[#00D4FF] transition-colors duration-200">
              <Icon name="lucide:linkedin" class="w-6 h-6" />
            </NuxtLink>
            <NuxtLink to="https://www.instagram.com/consultingelevation"
              class="text-gray-400 hover:text-[#00D4FF] transition-colors duration-200">
              <Icon name="lucide:instagram" class="w-6 h-6" />
            </NuxtLink>
          </div>
        </div>

        <!-- Quick Links -->
        <div>
          <h3 class="text-lg font-semibold mb-6 text-white border-b border-[#00D4FF] pb-2">Quick Links</h3>
          <ul class="space-y-3">
            <li>
              <NuxtLink to="/who-we-are" class="text-gray-300 hover:text-[#00D4FF] transition-colors duration-200">
                Who We Are
              </NuxtLink>
            </li>
            <li>
              <NuxtLink to="/corrosion-integrity-management"
                class="text-gray-300 hover:text-[#00D4FF] transition-colors duration-200">
                Corrosion & Integrity Management
              </NuxtLink>
            </li>
            <li>
              <NuxtLink to="/products-applications"
                class="text-gray-300 hover:text-[#00D4FF] transition-colors duration-200">
                Products & Applications
              </NuxtLink>
            </li>
            <li>
              <NuxtLink to="/marine-vessel" class="text-gray-300 hover:text-[#00D4FF] transition-colors duration-200">
                Marine Vessel Operations
              </NuxtLink>
            </li>
            <li>
              <NuxtLink to="/supply-chain" class="text-gray-300 hover:text-[#00D4FF] transition-colors duration-200">
                Supply Chain Management
              </NuxtLink>
            </li>
            <li>
              <NuxtLink to="/commitment" class="text-gray-300 hover:text-[#00D4FF] transition-colors duration-200">
                Our Commitment
              </NuxtLink>
            </li>
            <li>
              <NuxtLink to="/careers" class="text-gray-300 hover:text-[#00D4FF] transition-colors duration-200">
                Careers
              </NuxtLink>
            </li>
          </ul>
        </div>

        <!-- Services -->
        <div>
          <h3 class="text-lg font-semibold mb-6 text-white border-b border-[#00D4FF] pb-2">Our Services</h3>
          <ul class="space-y-3">
            <li>
              <NuxtLink to="/engineering"
                class="text-gray-300 hover:text-[#00D4FF] transition-colors duration-200 flex items-center justify-center md:justify-start">
                <Icon name="lucide:hard-hat" class="w-4 h-4 mr-2 text-[#00D4FF]" />
                Engineering
              </NuxtLink>
            </li>
            <li>
              <NuxtLink to="/construction"
                class="text-gray-300 hover:text-[#00D4FF] transition-colors duration-200 flex items-center justify-center md:justify-start">
                <Icon name="lucide:building" class="w-4 h-4 mr-2 text-[#00D4FF]" />
                Construction
              </NuxtLink>
            </li>
            <li>
              <NuxtLink to="/maintenance"
                class="text-gray-300 hover:text-[#00D4FF] transition-colors duration-200 flex items-center justify-center md:justify-start">
                <Icon name="lucide:settings" class="w-4 h-4 mr-2 text-[#00D4FF]" />
                Maintenance
              </NuxtLink>
            </li>
            <li>
              <NuxtLink to="/oil-and-gas"
                class="text-gray-300 hover:text-[#00D4FF] transition-colors duration-200 flex items-center justify-center md:justify-start">
                <Icon name="lucide:droplets" class="w-4 h-4 mr-2 text-[#00D4FF]" />
                Oil & Gas
              </NuxtLink>
            </li>
          </ul>
        </div>

        <!-- Contact Information -->
        <div>
          <h3 class="text-lg font-semibold mb-6 text-white border-b border-[#00D4FF] pb-2">Contact Us</h3>
          <ul class="space-y-4">
            <li class="flex items-start justify-center md:justify-start">
              <Icon name="lucide:map-pin" class="w-5 h-5 mt-0.5 mr-3 text-[#00D4FF] flex-shrink-0" />
              <span class="text-gray-300 text-left">120 Lewis Street Lagos Island,
                Lagos, Nigeria</span>
            </li>
            <li class="flex items-center justify-center md:justify-start">
              <Icon name="lucide:phone" class="w-5 h-5 mr-3 text-[#00D4FF] flex-shrink-0" />
              <span class="text-gray-300">(+234) 803 337 7366, <br>(+234) 803 878 6056</br><br>(+234)
                902 694 4592</br></span>
            </li>
            <li class="flex items-center justify-center md:justify-start">
              <Icon name="lucide:mail" class="w-5 h-5 mr-3 text-[#00D4FF] flex-shrink-0" />
              <span class="text-gray-300">info@elevationconsulting.com.ng,<br>
                elevationconsultingsg@gmail.com</br></span>
            </li>
            <li class="flex items-center justify-center md:justify-start">
              <Icon name="lucide:clock" class="w-5 h-5 mr-3 text-[#00D4FF] flex-shrink-0" />
              <span class="text-gray-300">Mon - Fri: 8:00 AM - 6:00 PM</span>
            </li>
          </ul>
        </div>
      </div>
      <!-- Bottom Footer with Darker Background -->
      <div class="bg-gray-950 -mx-4 sm:-mx-6 lg:-mx-8 px-4 -my-6 sm:px-6 lg:px-8 border-t border-gray-700 pt-6 pb-6">
        <div
          class="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0 text-center md:text-left">
          <p class="text-gray-400 text-sm">
            © {{ new Date().getFullYear() }} Elevation Consulting. All rights reserved.
          </p>
          <div class="flex flex-col sm:flex-row items-center space-y-2 sm:space-y-0 sm:space-x-6">
            <button @click="showPrivacyPolicy = true"
              class="text-gray-400 hover:text-white transition-colors duration-200 text-sm">
              Privacy Policy
            </button>
            <button @click="showTermsOfService = true"
              class="text-gray-400 hover:text-white transition-colors duration-200 text-sm">
              Terms of Service
            </button>
            <div class="text-gray-400 text-sm">
              Design & Development by
              <NuxtLink to="https://tekfol.io" target="_blank"
                class="text-[#00D4FF] hover:text-white transition-colors duration-200">
                Tekfolio
              </NuxtLink>
            </div>
          </div>
        </div>
      </div>

      <!-- Modal Components -->
      <PrivacyPolicyModal v-model="showPrivacyPolicy" />
      <TermsOfServiceModal v-model="showTermsOfService" />
    </div>
  </footer>
</template>

<script setup>
import { ref } from 'vue';
import PrivacyPolicyModal from '~/components/home/PrivacyPolicyModal.vue';
import TermsOfServiceModal from '~/components/home/TermsOfServiceModal.vue';

const showPrivacyPolicy = ref(false);
const showTermsOfService = ref(false);
</script>