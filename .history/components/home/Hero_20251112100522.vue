    ...
    <div class="absolute inset-0 z-10 flex items-center pt-4 md:pt-20 pb-4 md:pb-8">
      <div class="container mx-auto px-4 md:px-8 lg:px-12 h-full max-h-screen overflow-y-auto">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-12 items-center min-h-full py-4 md:py-0">
          ...
          <div
            class="relative h-[280px] sm:h-[300px] md:h-[350px] lg:h-[500px] order-1 lg:order-2 flex items-center justify-center pt-4 md:pt-0 overflow-hidden">
            <div class="book-display-container">
              <transition name="scroll-book" mode="out-in">
...
// Rotate books every 2.5 seconds (faster)
const rotateBooks = () => {
...
/* Barely noticeable crossfade transition for headlines */
...
/* Scroll-like transition for books */
.scroll-book-enter-active,
.scroll-book-leave-active {
  transition: all 0.6s ease-in-out;
}

.scroll-book-enter-from {
  opacity: 0;
  transform: translateY(40px);
}

.scroll-book-leave-to {
  opacity: 0;
  transform: translateY(-40px);
}

.scroll-book-enter-to,
.scroll-book-leave-from {
  opacity: 1;
