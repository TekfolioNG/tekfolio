      ...
      <div class="container mx-auto px-4 md:px-8 lg:px-12 h-full max-h-screen overflow-y-auto">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-12 items-center min-h-full py-4 md:py-0">
          ...
          <div class="text-center lg:text-left order-2 lg:order-1 space-y-3 md:space-y-4">
            <!-- Animated Headlines -->
            <div class="space-y-1 md:space-y-2 h-24 md:h-28 lg:h-32 relative">
              <transition name="crossfade" mode="out-in">
                <h1 v-if="currentHeadline === 1" key="headline1"
                  class="text-white text-xl sm:text-2xl md:text-3xl lg:text-4xl xl:text-5xl font-bold leading-tight drop-shadow-lg absolute inset-0">
          ...
          <div
            ...
            <div class="book-display-container">
              <transition name="scroll-horizontal" mode="out-in">
...
// Rotate books every 2 seconds (increased speed)
const rotateBooks = () => {
...
/* Barely noticeable crossfade for headlines */
...
/* Horizontal scroll transition for books - faster and scroll-like */
.scroll-horizontal-enter-active,
.scroll-horizontal-leave-active {
  transition: all 0.8s ease-in-out;
}

.scroll-horizontal-enter-from {
  opacity: 0;
  transform: translateX(60px) scale(0.95);
}

.scroll-horizontal-enter-to {
  opacity: 1;
  transform: translateX(0) scale(1);
}

.scroll-horizontal-leave-from {
  opacity: 1;
  transform: translateX(0) scale(1);
}

.scroll-horizontal-leave-to {
  opacity: 0;
  transform: translateX(-60px) scale(0.95);
}
