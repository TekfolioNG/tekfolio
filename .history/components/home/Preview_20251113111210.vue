<template>
    <section class="py-16 md:py-24 bg-gradient-to-br from-white to-blue-50/30 relative overflow-hidden">
        <!-- Background Elements -->
        <div class="absolute top-10 left-10 w-20 h-20 bg-blue-100/40 rounded-full blur-xl"></div>
        <div class="absolute bottom-10 right-10 w-32 h-32 bg-blue-200/30 rounded-full blur-2xl"></div>

        <div class="container mx-auto px-4 md:px-8 lg:px-12 relative z-10">
            <!-- Section Header -->
            <div class="text-center max-w-4xl mx-auto mb-16 md:mb-20">
                <div class="inline-flex items-center gap-2 bg-blue-50 px-4 py-2 rounded-full mb-6">
                    <div class="w-2 h-2 bg-[#4D7EFF] rounded-full"></div>
                    <span class="text-[#4D7EFF] font-semibold text-sm uppercase tracking-wide">Our Services</span>
                </div>
                <h2 class="text-4xl md:text-5xl lg:text-6xl font-bold text-gray-900 mb-6">
                    What We <span class="text-[#4D7EFF]">Do</span>
                </h2>
                <p class="text-xl md:text-2xl text-gray-700 font-semibold mb-8">
                    Clarity, Structure & Impact that Lasts
                </p>
                <p class="text-lg text-gray-800 leading-relaxed max-w-3xl mx-auto">
                    At Litera-Forge, we bridge strategy and creativity, enabling businesses, leaders, and authors bring
                    clarity to their ideas, structure to their vision, and the execution support they need to move
                    forward.

                </p>
                <p class="text-lg text-gray-800 leading-relaxed max-w-3xl mx-auto">
                    Our work spans four core areas that drive growth and transformation:
                </p>
            </div>

            <!-- Service Grid with Interactive Cards -->
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8 max-w-7xl mx-auto">
                <!-- Business Advisory & Restructuring -->
                <div
                    class="group relative bg-white rounded-2xl p-8 transition-all duration-500 hover:transform hover:scale-[1.02] border border-blue-100/50 hover:border-[#4D7EFF]/20 hover:shadow-2xl hover:shadow-blue-500/10">
                    <div
                        class="absolute inset-0 bg-gradient-to-br from-blue-50/50 to-transparent rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                    </div>
                    <div class="relative z-10">
                        <div class="flex items-start gap-6 mb-4">
                            <div
                                class="flex-shrink-0 w-16 h-16 bg-gradient-to-br from-[#4D7EFF] to-blue-400 rounded-2xl flex items-center justify-center group-hover:shadow-lg group-hover:shadow-blue-500/25 transition-all duration-500">
                                <TrendingUp class="w-8 h-8 text-white" />
                            </div>
                            <div class="flex-1">
                                <h3
                                    class="text-2xl font-bold text-gray-900 mb-3 group-hover:text-[#4D7EFF] transition-colors duration-300">
                                    Business Advisory & Restructuring
                                </h3>
                                <p class="text-gray-600 leading-relaxed text-lg">
                                    Strategic guidance and organizational alignment to help you operate efficiently,
                                    stay agile, and grow sustainably.
                                </p>
                            </div>
                        </div>
                        <div
                            class="flex items-center text-[#4D7EFF] font-semibold mt-6 opacity-0 group-hover:opacity-100 transform translate-x-0 group-hover:translate-x-2 transition-all duration-500">
                            <span>Explore advisory</span>
                            <ArrowRight class="w-5 h-5 ml-2" />
                        </div>
                    </div>
                </div>

                <!-- Employee Productivity Training -->
                <div
                    class="group relative bg-white rounded-2xl p-8 transition-all duration-500 hover:transform hover:scale-[1.02] border border-blue-100/50 hover:border-[#4D7EFF]/20 hover:shadow-2xl hover:shadow-blue-500/10">
                    <div
                        class="absolute inset-0 bg-gradient-to-br from-blue-50/50 to-transparent rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                    </div>
                    <div class="relative z-10">
                        <div class="flex items-start gap-6 mb-4">
                            <div
                                class="flex-shrink-0 w-16 h-16 bg-gradient-to-br from-[#4D7EFF] to-blue-400 rounded-2xl flex items-center justify-center group-hover:shadow-lg group-hover:shadow-blue-500/25 transition-all duration-500">
                                <Users class="w-8 h-8 text-white" />
                            </div>
                            <div class="flex-1">
                                <h3
                                    class="text-2xl font-bold text-gray-900 mb-3 group-hover:text-[#4D7EFF] transition-colors duration-300">
                                    Employee Productivity Training
                                </h3>
                                <p class="text-gray-600 leading-relaxed text-lg">
                                    Interactive sessions that help teams work smarter, collaborate better, and perform
                                    at their best.
                                </p>
                            </div>
                        </div>
                        <div
                            class="flex items-center text-[#4D7EFF] font-semibold mt-6 opacity-0 group-hover:opacity-100 transform translate-x-0 group-hover:translate-x-2 transition-all duration-500">
                            <span>Discover training</span>
                            <ArrowRight class="w-5 h-5 ml-2" />
                        </div>
                    </div>
                </div>

                <!-- Ghostwriting Services -->
                <div
                    class="group relative bg-white rounded-2xl p-8 transition-all duration-500 hover:transform hover:scale-[1.02] border border-blue-100/50 hover:border-[#4D7EFF]/20 hover:shadow-2xl hover:shadow-blue-500/10">
                    <div
                        class="absolute inset-0 bg-gradient-to-br from-blue-50/50 to-transparent rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                    </div>
                    <div class="relative z-10">
                        <div class="flex items-start gap-6 mb-4">
                            <div
                                class="flex-shrink-0 w-16 h-16 bg-gradient-to-br from-[#4D7EFF] to-blue-400 rounded-2xl flex items-center justify-center group-hover:shadow-lg group-hover:shadow-blue-500/25 transition-all duration-500">
                                <PenTool class="w-8 h-8 text-white" />
                            </div>
                            <div class="flex-1">
                                <h3
                                    class="text-2xl font-bold text-gray-900 mb-3 group-hover:text-[#4D7EFF] transition-colors duration-300">
                                    Ghostwriting Services
                                </h3>
                                <p class="text-gray-600 leading-relaxed text-lg">
                                    Transform your insights, stories, and expertise into powerful written assets—books,
                                    proposals, and profiles.
                                </p>
                            </div>
                        </div>
                        <div
                            class="flex items-center text-[#4D7EFF] font-semibold mt-6 opacity-0 group-hover:opacity-100 transform translate-x-0 group-hover:translate-x-2 transition-all duration-500">
                            <span>Start writing</span>
                            <ArrowRight class="w-5 h-5 ml-2" />
                        </div>
                    </div>
                </div>

                <!-- Book Publishing -->
                <div
                    class="group relative bg-white rounded-2xl p-8 transition-all duration-500 hover:transform hover:scale-[1.02] border border-blue-100/50 hover:border-[#4D7EFF]/20 hover:shadow-2xl hover:shadow-blue-500/10">
                    <div
                        class="absolute inset-0 bg-gradient-to-br from-blue-50/50 to-transparent rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                    </div>
                    <div class="relative z-10">
                        <div class="flex items-start gap-6 mb-4">
                            <div
                                class="flex-shrink-0 w-16 h-16 bg-gradient-to-br from-[#4D7EFF] to-blue-400 rounded-2xl flex items-center justify-center group-hover:shadow-lg group-hover:shadow-blue-500/25 transition-all duration-500">
                                <BookOpen class="w-8 h-8 text-white" />
                            </div>
                            <div class="flex-1">
                                <h3
                                    class="text-2xl font-bold text-gray-900 mb-3 group-hover:text-[#4D7EFF] transition-colors duration-300">
                                    Book Publishing
                                </h3>
                                <p class="text-gray-600 leading-relaxed text-lg">
                                    End-to-end publishing support that turns your manuscript into a polished,
                                    market-ready book.
                                </p>
                            </div>
                        </div>
                        <div
                            class="flex items-center text-[#4D7EFF] font-semibold mt-6 opacity-0 group-hover:opacity-100 transform translate-x-0 group-hover:translate-x-2 transition-all duration-500">
                            <span>Publish now</span>
                            <ArrowRight class="w-5 h-5 ml-2" />
                        </div>
                    </div>
                </div>
            </div>

            <!-- Enhanced CTA -->
            <div class="text-center mt-16 md:mt-20">
                <div
                    class="bg-gradient-to-r from-[#4D7EFF] to-blue-600 rounded-2xl p-8 md:p-12 max-w-4xl mx-auto shadow-2xl shadow-blue-500/25">
                    <h3 class="text-2xl md:text-3xl font-bold text-white mb-4">
                        Ready to Transform Your Vision into Reality?
                    </h3>
                    <p class="text-blue-100 text-lg mb-8 max-w-2xl mx-auto">
                        Let's discuss how our integrated approach can bring clarity, structure, and lasting impact to
                        your projects.
                    </p>
                    <div class="flex flex-col sm:flex-row gap-4 justify-center">
                        <NuxtLink to="/services"
                            class="inline-flex items-center justify-center gap-3 px-8 py-4 bg-white text-[#4D7EFF] font-bold text-lg rounded-xl transition-all duration-300 transform hover:scale-105 hover:shadow-2xl">
                            Explore All Services
                            <ArrowRight class="w-5 h-5" />
                        </NuxtLink>
                        <NuxtLink to="/book-session"
                            class="inline-flex items-center justify-center gap-3 px-8 py-4 bg-transparent border-2 border-white text-white font-bold text-lg rounded-xl transition-all duration-300 transform hover:scale-105 hover:bg-white hover:text-[#4D7EFF]">
                            Book a Discovery Call
                            <Calendar class="w-5 h-5" />
                        </NuxtLink>
                    </div>
                </div>
            </div>
        </div>
    </section>
</template>

<script setup>
import { ArrowRight, BookOpen, Calendar, PenTool, TrendingUp, Users } from 'lucide-vue-next';
</script>

<style scoped>
/* Custom smooth transitions */
.transition-all {
    transition-property: all;
    transition-timing-function: cubic-bezier(0.4, 0, 0.2, 1);
    transition-duration: 500ms;
}

/* Enhanced hover effects */
.group:hover .group-hover\:scale-105 {
    transform: scale(1.05);
}
</style>