...
onMounted(() => {
  ...
  const setupWaveEffect = async (buttonRef) => {
  ...
  let cleanupSecondary = null;

  // Setup wave effects after DOM is ready
  setupWaveEffect(primaryBtn).then(cleanup => {
    cleanupPrimary = cleanup;
  });
  
  setupWaveEffect(secondaryBtn).then(cleanup => {
    cleanupSecondary = cleanup;
  });
