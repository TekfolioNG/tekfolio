<template>
  <main class="relative w-screen h-screen overflow-hidden">
    ...
    <div class="absolute top-0 left-0 right-0 z-30 flex justify-center pt-28 md:pt-32">
      <transition name="button-slide" mode="out-in">
        <NuxtLink :key="'button-' + currentSlide" :to="slides[currentSlide].buttonLink"
          class="px-8 py-3 text-white font-regular transition-all duration-300 text-lg transform inline-block find-out-more-btn group"
          style="background: rgba(0, 0, 0, 0.6); backdrop-filter: blur(4px);">
          <span class="flex items-center">
            FIND OUT MORE
            <svg class="w-5 h-5 ml-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300" fill="none"
    ...
    <div class="absolute bottom-0 left-0 right-0 z-20 transition-all duration-600 content-panel"
      :class="currentSlide % 2 === 0 ? 'bg-gray-900/70 dark:bg-gray-900/80' : 'bg-red-600/70 dark:bg-red-700/80'"
      style="backdrop-filter: blur(4px);">
      <div class="container mx-auto px-6 py-6 text-center">
        <transition name="content-slide" mode="out-in">
          <div :key="'content-' + currentSlide">
            <!-- Category kicker -->
            <h3 class="text-white/90 text-lg md:text-xl font-barlow-extrabold tracking-wider uppercase mb-2">
              {{ slides[currentSlide].category }}
            </h3>
            <!-- Main heading -->
            <h2
              class="text-white text-3xl md:text-4xl lg:text-5xl font-barlow-thin leading-none max-w-3xl mx-auto whitespace-nowrap overflow-hidden text-ellipsis">
...
/* Custom font classes for better control */
.font-barlow-thin {
...
@media (max-width: 767px) {
  ...
  /* Adjust text size for mobile to prevent wrapping */
  .text-3xl {
    font-size: 1.625rem;
  }
