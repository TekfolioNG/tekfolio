...
<template>
  <div class="fixed w-full z-50 top-0 left-0 right-0 transition-all duration-300"
    :class="{ 'bg-white/90 shadow-md': scrolled && !isDark, 'bg-gray-900/90 shadow-md': scrolled && isDark }">
    <div class="container mx-auto px-4 sm:px-6 lg:px-8">
      <header class="flex items-center justify-between py-2">
        <!-- Logo section without background -->
        <a href="/" class="inline-block">
          <img src="~/assets/img/tekfolio-logo-final3.svg" alt="Tekfolio Logo" class="h-16 md:h-18 drop-shadow-lg" />
        </a>
        ...
        <div class="hidden md:flex items-center">
          <ClientOnly v-if="!colorMode?.forced">
            <div style="background-color: #00348F
;" class="rounded-lg p-1">
        ...
        <div class="md:hidden flex items-center">
          ...
          <ClientOnly v-if="!colorMode?.forced" class="mr-2">
            <div style="background-color: #00348F;" class="rounded-lg p-0.5">
      ...
      <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95"
        ...
        <div v-show="open" class="md:hidden mt-2 py-2 rounded-lg shadow-lg"
          :class="{ 'bg-white/90': !isDark, 'bg-gray-900/90': isDark }">
          <div v-for="item in menuitems" :key="item.title" class="block">
            <NuxtLink :to="item.path" class="block px-4 py-3 text-sm font-medium flex justify-between items-center"
              :class="{
                'text-gray-900 hover:text-blue-300': !isDark,
                'text-gray-100 hover:text-blue-300': isDark
              }" @click="open = false">
              {{ item.title }}
...
/* Ensure mobile menu has correct styles */
.bg-white\/90 a,
.bg-gray-900\/90 a {
