...
<template>
  <div class="fixed w-full z-50 top-0 left-0 right-0 bg-transparent">
    <div class="container mx-auto px-4 sm:px-6 lg:px-8">
      <header class="flex items-center justify-between py-2 bg-transparent">
        ...
        <nav
          class="hidden md:flex items-center justify-center absolute left-1/2 transform -translate-x-1/2 bg-transparent">
          <ul class="flex space-x-6 lg:space-x-8 bg-transparent">
            <li v-for="item in menuitems" :key="item.title" class="bg-transparent">
              <NuxtLink :to="item.path"
                class="text-sm lg:text-base transition-colors duration-200 flex items-center whitespace-nowrap font-medium bg-transparent"
                :class="{
                  'text-white': isDark || !isDark,
        ...
        <!-- Color Mode Toggle -->
        <div class="hidden md:flex items-center bg-transparent">
        ...
        <!-- Mobile Menu Button -->
        <div class="md:hidden flex items-center bg-transparent">
      ...
      <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95"
        ...
        <div v-show="open" class="md:hidden mt-2 py-2 rounded-lg shadow-lg"
          :class="{ 'bg-transparent': !isDark, 'bg-gray-900/90': isDark }">
          <div v-for="item in menuitems" :key="item.title" class="block">
            <NuxtLink :to="item.path" class="block px-4 py-3 text-sm font-medium flex justify-between items-center"
              :class="{
                'text-gray-100 hover:text-blue-300': isDark || !isDark
              }" @click="open = false">
              {{ item.title }}
...
<style scoped>
/* Ensure all elements are transparent by default */
.fixed {
