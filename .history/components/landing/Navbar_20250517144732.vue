  ...
  <div class="w-full z-50 top-0 left-0 right-0 transition-all duration-300">
    <div class="container mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95"
        ...
        <div v-show="open" class="md:hidden mt-2 py-2 rounded-lg shadow-lg backdrop-blur-md"
          :class="{ 'bg-transparent': !isDark, 'bg-gray-900/40': isDark }">
          <div v-for="item in menuitems" :key="item.title" class="block">
            <NuxtLink :to="item.path" class="block px-4 py-3 text-sm font-medium flex justify-between items-center"
              :class="{
                'text-white hover:text-blue-300': true
              }" @click="open = false">
              {{ item.title }}
...
<style scoped>
/* Make the navbar background fully transparent */
.fixed {
