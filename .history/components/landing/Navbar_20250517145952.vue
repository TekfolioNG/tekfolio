...
<template>
  <div class="w-full z-50 relative transition-all duration-300 bg-transparent">
    <div class="container mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95"
        ...
        <div v-show="open" class="md:hidden mt-2 py-2 rounded-lg shadow-lg backdrop-blur-md bg-transparent">
...
<style scoped>
/* Remove any background color based on color mode */
.bg-transparent,
.bg-gray-900\/40 {
  background-color: transparent !important;
}

/* Make the navbar background fully transparent regardless of color mode */
.fixed {
  background-color: transparent !important;
}

/* Add a glass effect when menu is open on mobile */
.backdrop-blur-md {
...
:deep(.router-link-active) {
  ...
  /* Light blue color matching hover */
  font-weight: 700;
  /* Bold font for active items */
}

/* Ensure mobile menu has correct styles regardless of color mode */
a {
...
@media (max-width: 767px) {
  ...
  /* Always use white text for better visibility regardless of color mode */
  a.block {
