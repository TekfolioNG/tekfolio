...
<template>
  <div class="w-full z-50 top-0 left-0 right-0 bg-transparent">
          ...
          <div v-for="item in menuitems" :key="item.title" class="block">
            <NuxtLink :to="item.path" class="block px-4 py-3 text-sm font-medium flex justify-between items-center"
              :class="{
                ...
                'text-gray-800 hover:text-blue-600': !isDark
              }" @click="open = false">
              {{ item.title }}
...
/* Text coloring based on mode */
.text-white {
  @apply text-white dark:text-white;
}

/* In light mode, we want the text to have better contrast against transparent bg */
:root.light .text-white {
  color: rgba(15, 23, 42, 0.9) !important;
  /* dark slate color for visibility */
}

/* When scrolling in light mode, use darker text */
:root.light .fixed .text-white {
  color: rgba(15, 23, 42, 0.9) !important;
}
...
:root.light :deep(.router-link-active) {
