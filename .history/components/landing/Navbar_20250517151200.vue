          ...
          <ul class="flex space-x-6 lg:space-x-8">
            <li v-for="item in menuitems" :key="item.title">
              <NuxtLink :to="item.path"
                ...
                :class="[
                  isDark ? 'text-white hover:text-blue-300' : 'text-gray-800 hover:text-blue-600'
                ]">
                {{ item.title }}
        ...
        <div v-show="open" class="md:hidden mt-2 py-2 rounded-lg shadow-lg bg-gray-900/90">
          <div v-for="item in menuitems" :key="item.title" class="block">
            <NuxtLink :to="item.path" class="block px-4 py-3 text-sm font-medium flex justify-between items-center"
              :class="[
                isDark ? 'text-gray-100 hover:text-blue-300' : 'text-gray-800 hover:text-blue-600'
              ]" @click="open = false">
              {{ item.title }}
...
/* Text coloring based on mode - using standard Tailwind classes */
:root.dark .navbar-text {
  @apply text-white;
}

:root.light .navbar-text {
  @apply text-gray-800;
}

/* Fix for mobile elements */
.mobile-menu-item {
