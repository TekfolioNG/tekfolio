...
// Compute background class based on scroll position and color mode
const navbarBackgroundClass = computed(() => {
  if (!isScrolled.value) {
    return 'bg-transparent';
  }
  return isDark.value
    ? 'bg-gray-900/90' // Dark mode scrolled background
    : 'bg-white/95';   // Light mode scrolled background (75% opacity)
});

// Compute text color based on color mode
...
const menuitems = [
  ...
  { title: "BLOGS & RESOURCES", path: "/contact", hasDropdown: false },
];
...
const handleScroll = () => {
  isScrolled.value = window.scrollY > 10; // Change threshold as needed
};
  ...
  <div class="fixed w-full z-50 top-0 left-0 right-0 transition-colors duration-300" :class="navbarBackgroundClass">
    <div class="container mx-auto px-4 sm:px-6 lg:px-8">
      <header class="flex items-center py-2">
        ...
        <!-- Modified spacer to control left shift of right elements -->
        <div class="flex-grow flex justify-end">
          <!-- Desktop Navigation - shifted slightly to the left with flex-end -->
          <div class="hidden md:flex items-center justify-end w-4/5">
            <!-- Menu Items -->
            ...
            <!-- Contact Us button with increased spacing -->
            <NuxtLink to="/contact"
              class="px-4 py-2 rounded-md text-sm lg:text-base font-medium whitespace-nowrap transition-colors duration-200 mr-6"
              :class="[
                isDark
