      ...
      <header class="flex items-center py-2">
            ...
            <ul class="flex space-x-6 lg:space-x-8">
              <li v-for="item in menuitems" :key="item.title">
                <NuxtLink :to="item.path"
                  ...
                  :class="[
                    textColorClass,
                    'hover:text-[#042433] md:hover:text-[#1d8ad8]'
                  ]">
                  {{ item.title }}
        ...
        <div class="hidden md:flex items-center ml-auto">
          <NuxtLink to="/contact"
            ...
            :class="[
              isDark
                ? 'text-white hover:text-white hover:bg-[#042433] md:hover:bg-[#1d8ad8] border border-[#042433] md:border-[#1d8ad8]'
                : 'bg-[#042433] md:bg-[#01348F] text-white hover:bg-[#042433] md:hover:bg-[#1d8ad8]'
            ]">
            CONTACT US
          </NuxtLink>

          <ClientOnly v-if="!colorMode?.forced">
            <div class="rounded-lg p-1 bg-[#042433] md:bg-[#01348F]">
        ...
        <div class="md:hidden flex items-center ml-auto space-x-4">
          <ClientOnly v-if="!colorMode?.forced" class="mr-2">
            <div class="rounded-lg p-0.5 bg-[#042433] md:bg-[#1D4ED8]">
              <UButton :icon="isDark ? 'i-lucide-moon' : 'i-lucide-sun'" color="white" variant="ghost" size="sm"
                class="text-white hover:text-[#042433] md:hover:text-blue-300" @click="isDark = !isDark" />
            </div>
            <template #fallback>
              <div class="size-6" />
            </template>
      ...
      <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95"
        ...
        <div v-show="open" class="md:hidden mt-2 py-2 rounded-lg shadow-lg bg-gray-900/90">
          <div v-for="item in menuitems" :key="item.title" class="block">
            <NuxtLink :to="item.path"
              class="block px-4 py-3 text-sm font-medium flex justify-between items-center text-gray-100 hover:text-[#042433] md:hover:text-blue-300"
              @click="open = false">
              {{ item.title }}
          ...
          <div class="block px-4 py-3">
            <NuxtLink to="/contact" class="block px-4 py-2 rounded-md text-sm font-medium text-center" :class="[
              isDark
                ? 'text-[#042433] md:text-blue-700 hover:text-white hover:bg-[#042433] md:hover:bg-blue-700 border border-[#042433] md:border-blue-700'
                : 'bg-[#042433] md:bg-blue-700 text-white hover:bg-[#042433] md:hover:bg-blue-800'
            ]" @click="open = false">
              CONTACT US
            </NuxtLink>
          </div>
...
/* Active link styling - using #042433 for mobile */
:deep(.router-link-active) {
  color: #042433 !important;
  /* Changed to your specified color for mobile */
  font-weight: 700;
}
