        ...
        <div class="hidden md:flex items-center ml-auto">
          ...
          <ClientOnly v-if="!colorMode?.forced">
            <div class="flex items-center space-x-3">
              <!-- Sun Icon -->
              <svg class="w-4 h-4" :class="!isDark ? 'text-yellow-500' : textColorClass" fill="currentColor"
                viewBox="0 0 20 20">
                <path fill-rule="evenodd"
              ...
              <button @click="isDark = !isDark"
                ...
                <span class="inline-block h-4 w-4 transform rounded-full bg-white transition duration-300 ease-in-out"
                  :class="isDark ? 'translate-x-6' : 'translate-x-1'" />
              </button>

              <!-- Moon Icon -->
              <svg class="w-4 h-4" :class="isDark ? 'text-blue-400' : textColorClass" fill="currentColor"
                viewBox="0 0 20 20">
        ...
        <div class="md:hidden flex items-center ml-auto space-x-4">
          ...
          <ClientOnly v-if="!colorMode?.forced" class="mr-2">
            <div class="flex items-center space-x-2">
              <!-- Sun Icon -->
              <svg class="w-3 h-3" :class="!isDark ? 'text-yellow-500' : textColorClass" fill="currentColor"
                viewBox="0 0 20 20">
                <path fill-rule="evenodd"
                  d="M10 2a1 1 0 011 1v1a1 1 0 11-2 0V3a1 1 0 011-1zm4 8a4 4 0 11-8 0 4 4 0 018 0zm-.464 4.95l.707.707a1 1 0 001.414-1.414l-.707-.707a1 1 0 00-1.414 1.414zm2.12-10.607a1 1 0 010 1.414l-.706.707a1 1 0 11-1.414-1.414l.707-.707a1 1 0 011.414 0zM17 11a1 1 0 100-2h-1a1 1 0 100 2h1zm-7 4a1 1 0 011 1v1a1 1 0 11-2 0v-1a1 1 0 011-1zM5.05 6.464A1 1 0 106.465 5.05l-.708-.707a1 1 0 00-1.414 1.414l.707.707zm1.414 8.486l-.707.707a1 1 0 01-1.414-1.414l.707-.707a1 1 0 011.414 1.414zM4 11a1 1 0 100-2H3a1 1 0 000 2h1z"
                  clip-rule="evenodd" />
              ...
              <button @click="isDark = !isDark"
                ...
                :class="isDark ? 'bg-blue-600' : 'bg-gray-300'">
                <span class="inline-block h-3 w-3 transform rounded-full bg-white transition duration-300 ease-in-out"
                  :class="isDark ? 'translate-x-5' : 'translate-x-1'" />
              </button>

              <!-- Moon Icon -->
              <svg class="w-3 h-3" :class="isDark ? 'text-blue-400' : textColorClass" fill="currentColor"
                viewBox="0 0 20 20">
                <path d="M17.293 13.293A8 8 0 016.707 2.707a8.001 8.001 0 1010.586 10.586z" />
              </svg>
            </div>
        ...
        <div v-show="open" class="md:hidden mt-2 py-2 rounded-lg shadow-lg bg-gray-900/90">
          ...
          <div class="block px-4 py-3">
            <NuxtLink to="/contact"
              ...
              <div class="absolute inset-0 rounded-md pointer-events-none">
                <!-- Top border -->
                <div class="absolute top-0 left-0 h-0.5 bg-[#1d8ad8] rounded-t-md transition-all duration-300 ease-out"
                  :class="!isContactHovered && !isContactClicked ? 'w-full' : 'w-0'"></div>
                <!-- Right border -->
                <div
                  class="absolute top-0 right-0 w-0.5 bg-[#1d8ad8] rounded-tr-md transition-all duration-300 ease-out delay-75"
                  :class="!isContactHovered && !isContactClicked ? 'h-full' : 'h-0'"></div>
                <!-- Bottom border -->
                <div
                  class="absolute bottom-0 right-0 h-0.5 bg-[#1d8ad8] rounded-br-md transition-all duration-300 ease-out delay-150"
                  :class="!isContactHovered && !isContactClicked ? 'w-full' : 'w-0'"></div>
                <!-- Left border -->
                <div
                  class="absolute bottom-0 left-0 w-0.5 bg-[#1d8ad8] rounded-bl-md transition-all duration-300 ease-out delay-[225ms]"
                  :class="!isContactHovered && !isContactClicked ? 'h-full' : 'h-0'"></div>
              </div>
