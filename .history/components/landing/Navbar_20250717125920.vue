...
const navbarBackgroundClass = computed(() => {
  return 'bg-black/10 backdrop-blur-sm border-b border-white/10';
});
...
const menuitems = [
  { title: "COMPANY", path: "/#", hasDropdown: false },
  { title: "SERVICES", path: "/#", hasDropdown: true },
  { title: "INDUSTRIES", path: "/#", hasDropdown: false },
  { title: "TECHNOLOGIES", path: "/#", hasDropdown: false },
  { title: "CASE STUDIES", path: "#", hasDropdown: false },
];
    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8">
      <header class="flex items-center py-4">
        ...
        <NuxtLink to="/" class="inline-block">
          <div
            class="bg-white/40 backdrop-blur-sm rounded-lg p-2 shadow-lg transition-all duration-200 hover:scale-105 hover:bg-white/50">
      ...
      <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95"
        ...
        <div v-show="open" class="md:hidden mt-2 py-2 rounded-lg shadow-lg bg-gray-900/90 backdrop-blur-sm">
          <div v-for="item in menuitems" :key="item.title" class="block">
            <NuxtLink :to="item.path"
              class="block px-4 py-3 text-sm font-medium flex justify-between items-center text-white hover:text-[#00D4FF] transition-colors duration-200"
              @click="open = false">
              {{ item.title }}
...
/* Active link styling - consistent cyan for all screen sizes */
:deep(.router-link-active) {
  color: #00D4FF !important;
  font-weight: 700;
}

/* Ensure all navbar links are white by default */
:deep(.router-link-exact-active) {
  color: #00D4FF !important;
}

/* Override any existing link colors */
nav a {
