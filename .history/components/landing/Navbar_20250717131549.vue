...
// Subtle background with backdrop blur for better visibility
...
const menuitems = [
  { title: "COMPANY", path: "/about", hasDropdown: false },
  { title: "SERVICES", path: "/services", hasDropdown: true },
  { title: "INDUSTRIES", path: "/industries", hasDropdown: false },
  { title: "TECHNOLOGIES", path: "/technologies", hasDropdown: false },
  ...
  <div class="fixed w-full z-50 top-0 left-0 right-0 transition-all duration-300 ease-in-out" :class="[
    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8">
      <header class="flex items-center py-4">
        <!-- Left-aligned logo with better positioning -->
        <NuxtLink to="/" class="inline-block">
          <div
            class="bg-white/90 backdrop-blur-sm rounded-lg p-2 shadow-lg transition-all duration-200 hover:scale-105 hover:bg-white/95">
            <img src="~/assets/img/tekfolio-no-tagline.png" alt="Tekfolio Logo"
              class="h-8 md:h-10 logo-mobile-desaturated" />
          </div>
        </NuxtLink>
            ...
            <ul class="flex space-x-6 lg:space-x-8">
              <li v-for="item in menuitems" :key="item.title">
                <NuxtLink :to="item.path"
                  class="text-sm lg:text-base transition-colors duration-200 flex items-center whitespace-nowrap font-medium drop-shadow-sm text-white hover:text-[#00D4FF]">
                  {{ item.title }}
        ...
        <div class="hidden md:flex items-center ml-auto">
          <NuxtLink to="/contact"
            class="px-4 py-2 rounded-md text-sm lg:text-base font-medium whitespace-nowrap transition-all duration-200 mr-12 relative overflow-hidden group drop-shadow-sm"
            :class="[
              textColorClass,
            ...
            <div class="absolute bottom-0 right-4 h-0.5 bg-[#00D4FF rounded-full transition-all duration-300 ease-out"
              :class="[
          ...
          <ClientOnly v-if="!colorMode?.forced">
            <button @click="isDark = !isDark"
              ...
              :class="isDark ? 'bg-[#00D4FF]' : 'bg-[#00D4FF]'">
        ...
        <div class="md:hidden flex items-center ml-auto space-x-4">
          ...
          <ClientOnly v-if="!colorMode?.forced" class="mr-2">
            <button @click="isDark = !isDark"
              class="relative inline-flex h-5 w-9 items-center rounded-full transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 drop-shadow-sm"
          ...
          <button @click="open = !open" :class="textColorClass + ' drop-shadow-sm'">
      ...
      <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95"
        ...
        <div v-show="open" class="md:hidden mt-2 py-2 rounded-lg shadow-lg bg-gray-900/90 backdrop-blur-sm">
          ...
          <div class="block px-4 py-3">
            <NuxtLink to="/contact"
              ...
              :class="[
                isDark
                  ? 'text-[#00D4FF] hover:text-white border border-[#00D4FF] bg-transparent hover:bg-blue-700/10'
                  : 'bg-[#00D4FF] text-white hover:bg-[#00D4FF] border border-transparent',
                isContactClicked ? 'animate-pulse scale-95' : ''
              ...
              <div class="absolute inset-0 rounded-md pointer-events-none">
                <!-- Top border -->
                <div class="absolute top-0 left-0 h-0.5 bg-[#00D4FF] rounded-t-md transition-all duration-300 ease-out"
                  :class="!isContactHovered && !isContactClicked ? 'w-full' : 'w-0'"></div>
                <!-- Right border -->
                <div
                  class="absolute top-0 right-0 w-0.5 bg-[#00D4FF] rounded-tr-md transition-all duration-300 ease-out delay-75"
                  :class="!isContactHovered && !isContactClicked ? 'h-full' : 'h-0'"></div>
                <!-- Bottom border -->
                <div
                  class="absolute bottom-0 right-0 h-0.5 bg-[#00D4FF] rounded-br-md transition-all duration-300 ease-out delay-150"
                  :class="!isContactHovered && !isContactClicked ? 'w-full' : 'w-0'"></div>
                <!-- Left border -->
                <div
                  class="absolute bottom-0 left-0 w-0.5 bg-[#00D4FF] rounded-bl-md transition-all duration-300 ease-out delay-[225ms]"
                  :class="!isContactHovered && !isContactClicked ? 'h-full' : 'h-0'"></div>
              </div>
