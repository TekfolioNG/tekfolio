    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8">
      <header class="flex items-center py-4">
        ...
        <NuxtLink to="/" class="inline-block">
          <div class="transition-all duration-200 hover:scale-10">
            ...
            <ul class="flex space-x-6 lg:space-x-8">
              <li v-for="item in menuitems" :key="item.title">
                <NuxtLink :to="item.path"
                  class="text-sm lg:text-base transition-colors duration-200 flex items-center whitespace-nowrap font-medium drop-shadow-sm text-white hover:text-[#1A73E8]">
                  {{ item.title }}
        ...
        <div class="hidden md:flex items-center ml-auto">
          <NuxtLink to="/contact"
            ...
            <div class="absolute bottom-0 right-4 h-0.5 bg-[#0078d4] rounded-full transition-all duration-300 ease-out"
              :class="[
          ...
          <ClientOnly v-if="!colorMode?.forced">
            <button @click="isDark = !isDark"
              ...
              :class="isDark ? 'bg-[#0078d4]' : 'bg-[#0078d4]'">
      ...
      <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95"
        ...
        <div v-show="open" class="md:hidden mt-2 py-2 rounded-lg shadow-lg bg-gray-900/90 backdrop-blur-sm">
          <div v-for="item in menuitems" :key="item.title" class="block">
            <NuxtLink :to="item.path"
              class="block px-4 py-3 text-sm font-medium flex justify-between items-center text-gray-100 hover:text-[#1A73E8]"
              @click="open = false">
              {{ item.title }}
          ...
          <div class="block px-4 py-3">
            <NuxtLink to="/contact"
              ...
              :class="[
                isDark
                  ? 'text-[#0078d4] hover:text-white border border-[#0078d4] bg-transparent hover:bg-blue-700/10'
                  : 'bg-[#0078d4] text-white hover:bg-[##0078d4] border border-transparent',
                isContactClicked ? 'animate-pulse scale-95' : ''
              ...
              <div class="absolute inset-0 rounded-md pointer-events-none">
                <!-- Top border -->
                <div class="absolute top-0 left-0 h-0.5 bg-[#0078d4] rounded-t-md transition-all duration-300 ease-out"
                  :class="!isContactHovered && !isContactClicked ? 'w-full' : 'w-0'"></div>
                <!-- Right border -->
                <div
                  class="absolute top-0 right-0 w-0.5 bg-[#0078d4] rounded-tr-md transition-all duration-300 ease-out delay-75"
                  :class="!isContactHovered && !isContactClicked ? 'h-full' : 'h-0'"></div>
                <!-- Bottom border -->
                <div
                  class="absolute bottom-0 right-0 h-0.5 bg-[#0078d4] rounded-br-md transition-all duration-300 ease-out delay-150"
                  :class="!isContactHovered && !isContactClicked ? 'w-full' : 'w-0'"></div>
                <!-- Left border -->
                <div
